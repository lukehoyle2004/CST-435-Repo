...
def client(monkeypatch):
    ...
    def insert_dataset(name, n_rows, n_features, positive_rate, records, labels):
        counters["datasets"] += 1
        row = {
    ...
    def insert_run(dataset_id, hidden_dim, lr, batch_size, epochs, metrics, model_b64):
        counters["runs"] += 1
        row = {
            "id": counters["runs"],
    ...
    def insert_prediction(run_id, features, proba, label):
        row = {
    ...
    def predictions_for_run(run_id):
        return [p for p in store["predictions"] if p["run_id"] == run_id]
    ...
    monkeypatch.setattr(db, "predictions_for_run", predictions_for_run)
